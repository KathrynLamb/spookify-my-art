...
// // Fallback: Jimp (pure JS)
// // ---- Fallback: Jimp (pure JS, no native) ----
// type JimpImage = {
...
// const Jimp: JimpLike = (jimpNamed ?? jimpDefault) as JimpLike;
...
async function loadAdapter(): Promise<ImageAdapter> {
  ...
  // ---- Fallback: Jimp v1 (pure JS, object-based APIs) ----
  type JimpImage = {
    bitmap: { width: number; height: number };
    resize: (size: { w: number; h: number }) => JimpImage;
...
async function generatePrintGuidePDF(): Promise<Buffer> {
  ...
  const h = (t: string, c = purple) => {
    page.drawText(t, { x: 50, y, size: 16, font: bold, color: c });
    y -= 25;
  };
  const p = (t: string) => {
    page.drawText(t, { x: 50, y, size: 12, font, color: black });
    y -= 20;
  };
  const li = (t: string) => {
    page.drawText(t, { x: 60, y, size: 11, font, color: black });
    y -= 18;
  };
  ...
  y -= 40;
  p('Congratulations! Your spooky masterpiece is ready to haunt your walls.');
  p('Here’s everything you need to print at home or at your local print shop.');
  y -= 10;

  h("What's in Your Package");
  ['• MASTER file: Full resolution (for large prints at print shops)', '• 2:3, 3:4, 4:5, 5:7 ratios: Portrait & Landscape', '• A5, A4, A3: Perfect for standard frames', '• All files are 300 DPI, print-ready!'].forEach(li);
  y -= 5;

  h('Printing at Home (A5 - A4 sizes)');
  ['1. Use the A4 or A5 file', '2. Glossy photo paper for best results (matte works too)', '3. Printer settings: “Best/High Quality”, “Borderless” if available', '4. Print a test on regular paper first'].forEach(li);
  y -= 5;

  h('Taking to a Print Shop (A3 and larger)');
  ['1. Bring the MASTER file on a USB or email it', '2. Ask for: “High quality photo print on glossy or matte paper”', '3. Most shops can print up to 24×36" or larger', '4. Try: Staples, FedEx, local photo labs, or online'].forEach(li);
  y -= 5;

  h('Quick Size Reference');
  ['• Small frames (5×7", 8×10"): Use 5-7 or 4-5 files', '• Medium frames (11×14", 12×18"): Use 3-4 or 2-3 files', '• Large prints (16×20"+): Use MASTER file at print shop', '• Standard frames: Use A5, A4, or A3 files'].forEach(li);
