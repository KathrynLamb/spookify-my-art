...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Get first line item + product via listLineItems
    ...
    const line: Stripe.LineItem | undefined = lineItems.data?.[0]
    ...
    const productName = product?.name || line?.description || 'Spookified Wall Art'
    ...
    const info: SessionInfo = {
      ...
      title: productName,
      // prefer metadata fileUrl, fall back to product image
      fileUrl: (session.metadata?.fileUrl as string | undefined) || productImage || undefined,
      ...
      // if you later persist it to Session metadata, this will show up
      gelatoOrderId: (session.metadata?.gelatoOrderId as string | undefined) || undefined,
    }
