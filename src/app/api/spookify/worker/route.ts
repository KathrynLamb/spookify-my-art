...
async function loadAdapter(): Promise<ImageAdapter> {
  ...
  // ---- Jimp v1 fallback (object-based API) ----
  type JimpImage = {
    bitmap: { width: number; height: number };
  ...
  const conformTo = async (
    ...
    if (!opts.aspect) {
      ...
      const outNoAspect = await img.getBufferAsync(Jimp.MIME_PNG);
      log('jimp.output no-aspect bytes', outNoAspect.byteLength);
      return outNoAspect;
    }
    ...
    if (opts.mode === 'contain') {
      ...
      if (srcAspect > tgtAspect) {
        ...
        const outContainWide = await canvas.getBufferAsync(Jimp.MIME_PNG);
        log('jimp.output contain bytes', outContainWide.byteLength);
        return outContainWide;
      } else {
        const w = Math.round(targetH * srcAspect);
        ...
        const outContainTall = await canvas.getBufferAsync(Jimp.MIME_PNG);
        log('jimp.output contain bytes', outContainTall.byteLength);
        return outContainTall;
      }
    } else {
      // cover: crop to fit then resize to exact target
      let cropW = srcW;
      let cropH = srcH;
      if (srcAspect > tgtAspect) {
        cropH = srcH;
      ...
      const outCover = await img.getBufferAsync(Jimp.MIME_PNG);
      log('jimp.output cover bytes', outCover.byteLength);
      return outCover;
    }
  };
