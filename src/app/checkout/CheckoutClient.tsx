...
// allow window.paypal without casting to any
declare global {
  interface Window {
    paypal?: PayPalSDK;
  }
...
export default function CheckoutClient() {
  ...
  const frameColor = sp.get('frameColor') || ''; // optional
  ...
  const missing = !fileUrl || !imageId || !amount || !currency || !size || !orientation;
        ...
        try {
          ...
          const isPrintAtHome =
            ...
            productUid === 'print-at-home' ||
             /\bprint\s*at\s*home\b/i.test(title);
      
      // in onApprove (digital branch)
if (isPrintAtHome) {
  const captureRes = await fetch('/api/paypal/capture', {
    ...
    body: JSON.stringify({
      ...
      gelatoOrder: { product: 'print-at-home', currency, fileUrl },
    }),
  });

  const result = await captureRes.json();
  if (!captureRes.ok || !result.ok) throw new Error(result?.error || 'Payment capture failed');

  // ✅ tell thank-you this was a digital order
  router.push(
    `/thank-you?digital=1&orderId=${data.orderID}&imageId=${imageId}&fileUrl=${encodeURIComponent(fileUrl)}`
  );
  return;
}

      
          // ... physical poster/framed poster logic below ...
