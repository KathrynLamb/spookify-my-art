    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70">
      <div className="w-full max-w-lg rounded-xl border border-white/15 bg-[#0b0b0e] p-5 text-white shadow-xl">
        ...
        <div className="mt-5 flex items-center justify-end gap-2">
          ...
          <button
            onClick={onSubmit}
            disabled={submitting || !draft.email}
        ...
        <p className="mt-3 text-xs text-white/60">
          Thanks! There’s a small hiccup with checkout right now—send this in and we’ll be in touch shortly to complete your order.
        </p>
      </div>
    </div>
  );
...
function ProductsInner() {
  ...
  // Helper to ensure we have a public URL (if user came via dataURL)
  ...
  async function checkout(productTitle: string, variant: CardVariant, titleSuffix: string) {
    // If payments are disabled, open manual modal instead
    if (!PAYMENTS_ENABLED) {
      try {
        if (!fileUrlQP || !imageId) {
          alert('Missing fileUrl or imageId.');
          return;
        }
        ...
        setDraft({
          ...
          frameColor: (variant as any).frameColor || null,
          fileUrl: publicUrl,
          imageId,
          currency,
        });
    ...
    // Payments enabled → existing flow
    try {
      if (!canProceed) {
        alert('Missing fileUrl or imageId.');
        return;
      ...
      let fileUrl = fileUrlQP;
      if (!isHttpUrl(fileUrl)) {
        fileUrl = await ensurePublicUrl(fileUrl, imageId);
      }
  ...
  async function submitManualOrder() {
    try {
      ...
      // setTimeout(() => setManualOpen(false), 2000);
    } catch (e) {
      ...
      <div className="mx-auto max-w-7xl px-4 py-8">
        ...
        {!PAYMENTS_ENABLED && (
          <div className="mb-6 rounded-xl border border-yellow-500/20 bg-yellow-500/10 px-4 py-3 text-yellow-200">
            Quick note: online checkout is having a wobble. Use “Select” to send us your details and we’ll invoice you right away. 💌
          </div>
        )}
