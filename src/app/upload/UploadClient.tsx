...
/* ======================= Utilities ======================= */
function inferOrientation(s: string): 'Horizontal' | 'Vertical' | 'Square' | null {
    const t = s.toLowerCase();
    if (/(horizontal|landscape|wide|16:9|3:2|postcard)/.test(t)) return 'Horizontal';
    if (/(vertical|portrait|tall|2:3)/.test(t)) return 'Vertical';
    if (/(square|1:1)/.test(t)) return 'Square';
    return null;
  }
  
  function pickOrientationFrom(...parts: Array<string | undefined | null>) {
    for (const p of parts) {
      if (!p) continue;
      const o = inferOrientation(p);
      if (o) return o;
    }
    return null;
  }
...
// TODO FIX TO ENSRE ORIENTATION
  const canGenerate = (!!imageId || !cfg.needsPhoto) && !generating;

  console.log("Why cant we generate", imageId, cfg.needsPhoto, plan, generating )
  useEffect(() => {
...
const refreshPlanFromServer = async (id: string) => {
  try {
    const j = await fetchJsonWithDebug<{ plan?: Plan; productPlan?: ProductPlan | null }>(
      `/api/get-plan?id=${encodeURIComponent(id)}`
    );
    if (j?.plan) setPlan(j.plan);
    if ('productPlan' in j) setProductPlan(j.productPlan ?? null);   // <—
  } catch (e) {
  ...
  const send = async () => {
    ...
    try {
      ...
      if ('productPlan' in data) setProductPlan(data.productPlan ?? null);  // <—

      const maybeCfg = data.plan ?? extractConfigFrom(data.content);
      const cfgPlan: Plan = isPlan(maybeCfg) ? (maybeCfg as Plan) : null;
      let nextPlan = cfgPlan ?? plan ?? {};

      const inferred =
            pickOrientationFrom(
                userText,
                data.finalizedPrompt,
                typeof data.content === 'string' ? data.content : undefined,
                cfgPlan?.finalizedPrompt
            )

      if (inferred && !nextPlan.orientation) {
        nextPlan = {
          ...(nextPlan ?? {}),
            orientation: inferred,
          targetAspect:
            inferred === 'Horizontal' ? 1.4 :
            inferred === 'Vertical'   ? 0.7  : 1,
        };
      }
      
      if (cfgPlan || inferred) {
        setPlan(nextPlan as Plan);
        const short = summarizePlan(nextPlan as Plan, userText);
    ...
    try {
      ...
      const poll = async (): Promise<void> => {
        ...
        //   }
          ...
          if (j.status === 'done') {
            const url = j.resultUrl || j.resultFullUrl || j.previewUrl;
            if (url) {
              setResultUrl(url);
              setGenerating(false);
              stopped = true;
  ...
  useEffect(() => {
    if (!productPlan?.productId || plan?.orientation) return;
    const pid = productPlan.productId.toLowerCase();
    const inferred =
      pid.includes('landscape') ? 'Horizontal' :
      pid.includes('portrait')  ? 'Vertical'   :
      pid.includes('square')    ? 'Square'     : null;
    if (inferred) {
      setPlan(p => ({ ...(p ?? {}), orientation: inferred, targetAspect: inferred === 'Horizontal' ? 1.4 : inferred === 'Vertical' ? 0.7 : 1 }));
    }
  }, [productPlan, plan?.orientation]);
  

  const goChooseProduct = useCallback(async () => {
    // basic guards
    if (!resultUrl) { setError('No generated image found. Generate first.'); return; }
    if (!imageId)   { setError('Missing image id.'); return; }
  
    try {
      // ensure the preview/result is a public URL
      let handoffUrl = resultUrl;
      if (!/^https?:\/\//i.test(resultUrl)) {
        handoffUrl = await ensurePublicUrl(resultUrl, imageId);
      }
  
      const qp = new URLSearchParams({
        imageId,
        fileUrl: handoffUrl,
        source: productPlan?.productId ? 'recommend' : 'manual',
        mode: mode ?? '',
      });
  
      // pass along product hint + orientation if known
      if (productPlan?.productId) qp.set('productId', productPlan.productId);
      if (plan?.orientation && ['Horizontal', 'Vertical', 'Square'].includes(plan.orientation)) {
        qp.set('orientation', plan.orientation);
      }
  
      // if the URL might get long, stash a tiny handoff token as a fallback
      // (optional, safe to remove)
      if (handoffUrl.length > 1800) {
        const token = `handoff:${Date.now()}`;
        localStorage.setItem(token, JSON.stringify({ imageId, fileUrl: handoffUrl, plan, productPlan }));
        qp.delete('fileUrl');
        qp.set('handoff', token);
      }
  
      startTransition(() => {
        // ❌ remove dismiss(); it doesn’t exist here
        router.push(`/selectedProduct?${qp.toString()}`); // ensure route casing matches your file
      });
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
    }
  }, [resultUrl, imageId, plan, productPlan, ensurePublicUrl, mode, router]);
...
{productPlan?.productId && (
  <div className="bg-white/5 border border-white/10 rounded-xl p-3 md:p-4">
    <div className="flex items-start justify-between gap-3">
      <div>
        <div className="text-sm uppercase tracking-wide text-white/50">Recommended product</div>
        <div className="mt-1 text-base font-semibold">
          {productPlan.productId.replaceAll('_', ' ')}
        </div>
        <div className="mt-1 text-white/80 text-sm">
          {productPlan.reasonShort || 'Perfect fit for your plan.'}
        </div>
      ...
      {/* tiny product thumb if you have one */}
      {/* <Image ... /> */}
    </div>
    ...
    <div className="mt-3 flex flex-wrap gap-2">
      <button
        onClick={goChooseProduct}
        disabled={generating}
        className="inline-flex items-center rounded-full bg-white text-black px-4 py-2 text-sm font-medium hover:opacity-90 disabled:opacity-50"
      >
        Use this product → Continue
      </button>

      <a
        href="/products"
        className="inline-flex items-center rounded-full border border-white/15 bg-white/5 px-4 py-2 text-sm hover:bg-white/10"
      >
        Browse all
      </a>
    </div>
